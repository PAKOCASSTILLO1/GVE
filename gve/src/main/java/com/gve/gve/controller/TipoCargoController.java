...
import com.gve.gve.repository.TipoCargoRepository;
...
public class TipoCargoController {
    ...
    public ResponseEntity<ApiResponse> listar(){
        try {
            ApiResponse apiResponse = new ApiResponse("OK", "Proceso Exitoso", tipoCargoRepository.findAll());
    ...
    public ResponseEntity<ApiResponse> ver(@PathVariable int id){
        try {
            List<TipoCargo> data = new ArrayList<>();
            data.add(tipoCargoRepository.findById(id).orElse(null));
    ...
    @PostMapping("/crear")
    public ResponseEntity<ApiResponse> ver(@RequestBody TipoCargo tipoCargo){
        try {
            List<TipoCargo> data = new ArrayList<>();
            data.add(tipoCargoRepository.save(tipoCargo));
            ApiResponse apiResponse = new ApiResponse("OK", "Proceso Exitoso", data);
            return new ResponseEntity<ApiResponse>(apiResponse, HttpStatus.OK);
        } catch (Exception e) {
            ApiResponse apiResponse = new ApiResponse("FAIL", e.getMessage(), null);
    ...
    // public ResponseEntity<ApiResponse> editar(@PathVariable int id, @RequestBody TipoCargo body){
    //     try {
    //         List<TipoCargo> data = new ArrayList<>();
    //         TipoCargo tipoCargo = tipoCargoRepository.findById(id).orElse(null);            
    //         data.add(tipoCargoRepository.save(tipoCargo.modified(body)));
    //         ApiResponse apiResponse = new ApiResponse("OK", "Proceso Exitoso", data);
    //         return new ResponseEntity<ApiResponse>(apiResponse, HttpStatus.OK);
    //     } catch (Exception e) {
    //         if (e.toString().equals("java.lang.NullPointerException")) {
